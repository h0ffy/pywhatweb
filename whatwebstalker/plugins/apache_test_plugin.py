...
class Pluginapache_test_plugin(plugins.Base):
    def __init__(self):
        pass
    ...
    def start(self):
        self.rules = [
            ...
            { "regexp" : "^Apache", "search" : "headers[server]", "name" : "HTTP Server Header"},
            { "version" : "^Apache\\/([\\d\\.]+)", "search" : "headers[server]", "name" : "HTTP Server Header"},
