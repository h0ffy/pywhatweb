...
class Pluginioncube_loader_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            ...
            { "version" : r'/<p>The ionCube Loader ([^\s]+) is already installed but it is an old version\. It is recommended that the Loader be updated to the latest version/' },
            { "version" : r'/<p>The ionCube Loader ([^\s]+) is already installed and encoded files should run without problems.<\/p>/' },
            { "module" : r'/<div id="footer">Copyright ionCube Ltd\. 2002-20[\d]{2} \| (Loader Wizard version [^\s]+) /' },
