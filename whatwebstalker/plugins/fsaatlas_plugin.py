...
class Pluginfsaatlas_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            { "version" : r"/<title>fsaATLAS Enterprise v\.([^\\s^<]+)<\/title>/" },
            { "version" : r'/<div style="position:absolute; width:100px; top:0px; right:0px"><img src="images\\/fsaatlastext\\.png" alt="fsaATLAS Enterprise v\.([^\\s^"]+)"\\/><\\/div>/' },
