...
class Pluginclipbucket_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            { "string" : r'<meta name="copyright" content="ClipBucket - PHPBucket ClipBucket 2007 - (20[\d]{2})" \/>' },
            { "text" : '<meta name="author" content="Arslan Hassan - http://clip-bucket.com/arslan-hassan" />' },
            { "text" : "<!-- ClipBucket v2 -->", "version" : "2.x" },
            { "version" : r'<!-- ClipBucket version ([\d\.]+) -->' },
            ...
            { "search" : "headers[set-cookie]", "regexp" : r"pageredir=https?%3A%2F%2F", "certainty" : "25" },
        ]
        return self.rules
```