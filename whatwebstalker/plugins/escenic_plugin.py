...
class Pluginescenic_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            ...
            { "certainty" : "25", "regexp" : r'/<form action="\/sok\/" id="[^"]+" method="get" accept-charset="utf-8"[^>]*>/' },
            { "certainty" : "75", "ghdb" : "filetype:ece inurl:article" },
            { "certainty" : "25", "regexp" : r'/<img[^>]+src="[^"^>]+\/archive\/\d{5}\/[^"^>]+"[^>]*>/' },
            { "certainty" : "25", "regexp" : r'/<a[^>]+href="[^"^>]+\/article[\d]{5,10}\.ece">[^<]*<\/a>/' },
        ]
        return self.rules