...
class Pluginelite_gaming_ladders_plugin(plugins.Base):
    def __init__(self):
        pass
    ...
    def start(self):
        self.rules = [
            ...
            { "regexp" : r'/<b><span style="color:#013171;">Design By: <\/span> Soulast<span style="color:#013171;"> \| Powered [bB]y: <\/span>[\\s]?<a href="http:\\/\\/(www\\.)?eliteladders\\.com"/' },
            { "version" : r"/Powered by: <a href='http:\\/\\/(www\\.)?eliteladders\\.com'>Elite Gaming Ladders (\\s|&nbsp;)?v([^<]+)<\/a>/", "offset" : "2" },
            { "version" : r"/Powered [bB]y:? <a href='http:\\/\\/(www\\.)?eliteladders\\.com'>[\\s]+<span style=['\"]color:#[^;]{6};['\"]>Elite Gaming Ladders v([^<]+)<\/span><\/a>/", "offset" : "1" },
        ]
        return self.rules
```