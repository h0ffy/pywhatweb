...
class Pluginbarracuda_spam_firewall_plugin(plugins.Base):
    def __init__(self):
        pass
    ...
    def start(self):
        self.rules = [
            ...
            { "version" : r'/<link rel="stylesheet" type="text\\/css" href="\\/barracuda.css\\?v=([\\d\\.]+)">/', "module" : "Virus Firewall" },
            { "version" : r'/<script language=javascript src="\\/js_functions.([\\d\\.]+).js" type="text\\/javascript"><\\/script>/' },
            { "firmware" : r'/<td align=left class=config_module valign=top><font size=-2 color=#aaaaaa>Serial #[A-Z]+-[A-Z]+-[\\d]+<br>Firmware v([\\d\\.]+) <font color=#ffffff>/' },
        ]
        return self.rules