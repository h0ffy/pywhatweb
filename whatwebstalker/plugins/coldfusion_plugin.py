...
class Plugincoldfusion_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            ...
            { "regexp" : r'/<meta name="Author" content="Copyright (\(c\)\ )?[0-9]{4}-[0-9]{4} Macromedia( Corp|, Inc)\. All rights reserved\.">/' },
            ...
            { "search" : "headers[page-completion-status]", "certainty" : "75", "regexp" : r'/(Abnormal|Normal)/' },
            { "search" : "headers[set-cookie]", "regexp" : r'/CFAUTHORIZATION_cfadmin=/' },
            { "name" : "File extension", "regexp" : r'/^(cfm|cfc)$/', "search" : "uri.extension" },
        ]
        return self.rules
```