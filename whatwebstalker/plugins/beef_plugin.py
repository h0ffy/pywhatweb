...
class Pluginbeef_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            ...
            { "regexp" : r"/<script[^>]+src=[\'"]?[^>^=\'^"]+\\/hook\\/beefmagic\\.js\\.php[\'"]?/", "certainty" : "75", "string" : "Hook" },
            { "regexp" : r"/<script[^>]+src=[\'"]?https?:\\/\\/[^\\/]+\\/hook\\.js[\'"]?/", "certainty" : "75", "string" : "Hook" },
            ...
            { "search" : "headers[server]", "version" : r"/^BeEF ([^\s]+)$/" },
        ]
        return self.rules
```