...
class Plugindatalife_engine_plugin(plugins.Base):
    ...
    def start(self):
        self.rules = [
            ...
            { "regexp" : r'/<script language="javascript" type="text/javascript">\s+<!--\s+var dle_root       = \'[^\']+\';\s+var dle_admin      = \'[^\']*\';\s+var dle_login_hash = \'[^\']*\';\s+(var dle_group      = [\d]+;\s+)?var dle_skin       = \'[^\']*\';\s+var dle_wysiwyg    = \'[^\']*\';\s+var quick_wysiwyg  = \'[^\']*\';\s+/' },
